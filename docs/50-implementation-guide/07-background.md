...
对应文件：`src/background/index.ts`、`src/background/backgroundApp.ts`
...
deps.addMessageListener((message, sendResponse) => {
  ...
  void broadcastToAo3Tabs(deps, message);
});
```

`backgroundApp.ts` 使用依赖注入实现可测试控制器：真实运行时注入 `chrome.runtime.onMessage`、`chrome.tabs.query`、`chrome.tabs.sendMessage`，测试中注入 mock。
...
export function initBackgroundApp(deps = realDeps): void {
  deps.addMessageListener((message, sendResponse) => {
    if (!isRuntimeMessage(message)) return;
...
async function broadcastToAo3Tabs(deps, message: RuntimeMessage): Promise<void> {
  ...
  for (const tab of tabs) {
    ...
    try {
      ...
      // 忽略没有 content script 或尚未 ready 的 tab
    }
  }
}
```
...
本阶段只保留规则和设置更新消息，不提前加入 popup 统计协议：
...
`backgroundApp.ts` 已通过依赖注入覆盖自动化测试：

- 收到 `RULES_UPDATED` / `SETTINGS_UPDATED` 会 query AO3 tabs 并广播
- 非目标消息不广播
- tab 缺少 id 时跳过
- 单个 tab 发送失败时继续广播其他 tab
